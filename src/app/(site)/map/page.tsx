import type { Metadata } from "next";
import { MapPageClient } from "@/components/map/MapPageClient";
import { JsonLd } from "@/components/seo/JsonLd";
import { getLocations } from "@/lib/data";
import { getLocalizedLocations } from "@/lib/location-i18n";
import { filterLocationsNearMarina } from "@/lib/map-geo";
import { getRequestLocale } from "@/lib/i18n/request-locale";
import {
  buildPageMetadata,
  getLocalizedPageSeo,
} from "@/lib/seo/build-metadata";
import { breadcrumbSchema, schemaGraph, webPageSchema } from "@/lib/seo/schemas";

export async function generateMetadata({
  searchParams,
}: {
  searchParams: Promise<{ lang?: string }>;
}): Promise<Metadata> {
  const { lang } = await searchParams;
  return buildPageMetadata("map", "/map", { lang });
}

export default async function MapPage({
  searchParams,
}: {
  searchParams: Promise<{ lang?: string }>;
}) {
  const { lang } = await searchParams;
  const locale = await getRequestLocale(lang);
  const seo = await getLocalizedPageSeo("map", lang);
  const locations = filterLocationsNearMarina(
    getLocalizedLocations(getLocations(), locale),
  );

  return (
    <>
      <JsonLd
        data={schemaGraph(
          webPageSchema({
            name: seo.title,
            description: seo.description,
            path: "/map",
            locale: seo.locale,
          }),
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "Map", path: "/map" },
          ]),
        )}
      />
      <MapPageClient locations={locations} />
    </>
  );
}
